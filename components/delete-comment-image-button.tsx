...
interface DeleteCommentImageButtonProps {
  ticketId: string
  commentId: string
  imageId: string
}

export function DeleteCommentImageButton({
  ticketId,
  commentId,
  imageId,
}: DeleteCommentImageButtonProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `/api/tickets/${ticketId}/comments/${commentId}/images/${imageId}`,
        { method: 'DELETE' }
      )
      ...
      console.error('Failed to delete comment image:', err)
      setIsDeleting(false)
